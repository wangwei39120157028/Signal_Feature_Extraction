...
def func(x,a,b,c):
    return a * np.power(x,3) + b * np.power(x,2) + c * np.power(x,1)
popt, pcov = curve_fit(func, W, R,[1,1,1])
...
plot2=plt.plot(W, yvals, 'r',label='curve_fit values')
plt.xlabel('W axis')
plt.ylabel('R axis')
...
popt2, pcov2 = curve_fit(func, W, J,[1,1,1])
a2=popt2[0] # popt里面是拟合系数，读者可以自己help其用法
b2=popt2[1]
c2=popt[2]
yvals2=func(W,a2,b2,c2)
print(popt2)
plot1=plt.plot(W, J, '*',label='original values')
plot2=plt.plot(W, yvals2, 'r',label='curve_fit values')
plt.xlabel('W axis')
plt.ylabel('J axis')
plt.legend(loc=4) # 指定legend的位置,读者可以自己help它的用法
plt.title('curve_fit')
...
popt, pcov = curve_fit(func, W, Z,[1,1,1])
a=popt[0] # popt里面是拟合系数，读者可以自己help其用法
b=popt[1]
c=popt[2]
yvals=func(W,a,b,c)
print(popt)
plot1=plt.plot(W, Z, '*',label='original values')
plot2=plt.plot(W, yvals, 'r',label='curve_fit values')
plt.xlabel('W axis')
plt.ylabel('Z axis')
plt.legend(loc=4) # 指定legend的位置,读者可以自己help它的用法
